...
private:
    ...
    void layoutPatientPuzzleUi() {
        ...
        if (m_patientRotateHint
            ...
            // 1.20: ~1350×300 (−100 влево); 1.19: рядом с зоной трафарета; 1.14/2: @ (1400,150).
            double hx = 1400.0;
            double hy = 150.0;
    ...
    void layoutStopAndHints() {
        ...
        if (m_exerciseId == QStringLiteral("1.20")) {
            // Поворот: −100 влево от 1450; по высоте с заданием (Y=300).
            placeStop(977.0, 70.0);
            placeRotateHint(1350.0, 300.0);
            return;
        }
        ...
        if (m_exerciseId == QStringLiteral("3.1.8")) {
            // Было 970@70; поднять Стоп на 40 px (оба задания).
        ...
        if (m_exerciseId == QStringLiteral("3.1.23")) {
            // Было 970@70; смещение Стоп на 500 px влево.
            placeStop(470.0, 70.0);
            return;
        }
        ...
        if (m_exerciseId == QStringLiteral("1.28")) {
            // Оригинал puzzles.cs: pstop @ 970; итого −1100 от прежних 1120 → 20.
            placeStop(20.0, 70.0);
            return;
        }
        m_stop->move(80, 72);
