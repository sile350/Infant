...
public:
    ...
    // После setHtml: жёстко 671px всем таблицам (шапка 2 кол. иначе чуть уже заданий).
    static void forceProtocolDocumentTableWidths(QTextDocument *document, int widthPx = 671);
    ...
    // 1.272: summary </table><!--s--> + таблица процесса (не вкладывать в «Примечание»).
    static QString canonicalizeProtocol1272StoredBody(const QString &protocolBody);
    ...
    static QString buildProtocol126ViewRecord(
        const QString &headerFragment,
        const QString &storedBody);
    static QString buildProtocol1272ViewRecord(
        const QString &headerFragment,
        const QString &storedBody);
