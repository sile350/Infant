...
QString normalizeSummaryColumnWidthsHtml(QString body) {
        ...
        while (it.hasNext()) {
            ...
            while (trIt.hasNext()) {
                ...
                if (tds.size() >= 1) {
                    ...
                    if (isProcessBanner) {
                        // Шапка 2 кол. — одна ячейка на всю ширину.
                        rowInner = QStringLiteral(
    ...
    // Повторная сессия (только Дата/Результат/Примечание, без «Методика»): тоже строго 2 колонки.
    // Иначе 3-кол. процесс после <!--s--> или старый colspan раздувает пустую колонку справа.
    {
        const QRegularExpression summaryTableRe(
            QStringLiteral(
                "(<table\\b[^>]*>)([\\s\\S]*?Дата\\s*/\\s*специалист[\\s\\S]*?)(</table>)"),
            QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption);
        QString out;
        out.reserve(body.size() + 64);
        int last = 0;
        ...
        while (it.hasNext()) {
            ...
            QString open = m.captured(1);
            QString inner = m.captured(2);
            if (inner.contains(QStringLiteral("Методика"), Qt::CaseInsensitive)
                || inner.contains(QStringLiteral("Факт выполнения"), Qt::CaseInsensitive)
    ...
    // «Процесс выполнения…»: Qt после roundtrip часто даёт несколько <td> вместо одной с colspan.
    // Важно: colspan = число колонок ТАБЛИЦЫ (в 5.4.2 процесс — 3 кол.; иначе colspan=2
    // оставляет узкую пустую колонку справа — как на скрине).
    {
        const QRegularExpression tableRe(
            QStringLiteral("(<table\\b[^>]*>)([\\s\\S]*?)(</table>)"),
            QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption);
        QString out;
        out.reserve(body.size() + 64);
        int last = 0;
        ...
        while (tableIt.hasNext()) {
            const QRegularExpressionMatch tm = tableIt.next();
            out += body.mid(last, tm.capturedStart() - last);
            QString inner = tm.captured(2);
            int tableCols = 2;
            const QRegularExpression colRe(
                QStringLiteral("<col\\b[^>]*>"), QRegularExpression::CaseInsensitiveOption);
            const int colCount = inner.count(colRe);
            if (colCount >= 2) {
                tableCols = colCount;
            } else if (inner.contains(QStringLiteral("Вопросы"), Qt::CaseInsensitive)
                       && (inner.contains(QStringLiteral("Ответы"), Qt::CaseInsensitive))
                       && inner.contains(QStringLiteral("Виды помощи"), Qt::CaseInsensitive)) {
                tableCols = 3;
            } else {
                const QRegularExpression trRe(
                    ...
                    QRegularExpression::CaseInsensitiveOption
                        | QRegularExpression::DotMatchesEverythingOption);
                QRegularExpressionMatchIterator trScan = trRe.globalMatch(inner);
                while (trScan.hasNext()) {
                    const QString rowInner = trScan.next().captured(1);
                    if (htmlFragmentToPlainText(rowInner).contains(
                            QStringLiteral("Процесс выполнения"), Qt::CaseInsensitive)) {
                        continue;
                    }
                    const int n = rowInner.count(
            ...
            while (trIt.hasNext()) {
                ...
                QString processInner;
                for (const QRegularExpressionMatch &td : tds) {
                    const QString cellPlain = htmlFragmentToPlainText(td.captured(3)).trimmed();
                    if (cellPlain.contains(QStringLiteral("Процесс выполнения"), Qt::CaseInsensitive)) {
                        processInner = td.captured(3);
                        break;
                    }
                }
                if (!processInner.isEmpty()) {
                    rowInner = QStringLiteral(
                        "<td colspan='%1' align='center' width='671' style='width:671px'>%2</td>")
                                   .arg(tableCols)
                                   .arg(processInner);
                    rebuilt += tr.captured(1) + rowInner + tr.captured(3);
                } else {
                    rebuilt += tr.captured(0);
                }
                trLast = tr.capturedEnd();
            }
            rebuilt += inner.mid(trLast);
            out += tm.captured(1) + rebuilt + tm.captured(3);
            last = tm.capturedEnd();
        }
        out += body.mid(last);
        body = out;
    ...
    // Таблицы процесса «Факт выполнения» (2.8/2.9 — 3 кол.; 2.10 — 4 кол.): сумма = 671.
    {
        ...
        while (it.hasNext()) {
            ...
            inner.remove(QRegularExpression(
                QStringLiteral("<colgroup\\b[\\s\\S]*?</colgroup>\\s*"),
                QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption));
            // 1.19: Изображение / Факт / Характер / Виды помощи — 185+95+196+195 = 671.
    ...
    // 3.1.21: № / Ответы (3) / Виды помощи / Баллы — 40+104×3+250+69 = 671.
    {
        const QRegularExpression processTableRe(
            QStringLiteral(
                "(<table\\b[^>]*>)([\\s\\S]*?Ответы\\s+ребенка[\\s\\S]*?Вопрос\\s*[№N]"
                "[\\s\\S]*?Виды\\s+помощи[\\s\\S]*?Баллы[\\s\\S]*?)(</table>)"),
            QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption);
        QString out;
        out.reserve(body.size() + 128);
        int last = 0;
        ...
        while (it.hasNext()) {
            ...
            QString open = m.captured(1);
            QString inner = m.captured(2);
            if (inner.contains(QStringLiteral("Характер деятельности"), Qt::CaseInsensitive)
            ...
            inner.remove(QRegularExpression(
                ...
                QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption));
            inner.prepend(QStringLiteral(
                "<colgroup><col width='40'><col width='104'><col width='104'>"
                "<col width='104'><col width='250'><col width='69'></colgroup>"));
            const QStringList widths6 = {
                QStringLiteral("40"), QStringLiteral("104"), QStringLiteral("104"),
                QStringLiteral("104"), QStringLiteral("250"), QStringLiteral("69")};
            const QRegularExpression trRe(
                QStringLiteral("(<tr\\b[^>]*>)([\\s\\S]*?)(</tr>)"),
                QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption);
            QString rebuilt;
            ...
            while (trIt.hasNext()) {
                ...
                if (tds.size() == 1
                    ...
                    QString attrs = setAttrWidth(tds.at(0).captured(2), QStringLiteral("671"));
                    if (!attrs.contains(QStringLiteral("colspan"), Qt::CaseInsensitive)) {
                        attrs += QStringLiteral(" colspan='6'");
                    }
                    rowInner = tds.at(0).captured(1) + attrs + tds.at(0).captured(3)
                        + tds.at(0).captured(4) + tds.at(0).captured(5);
                } else if (tds.size() == 4
                           && tds.at(0).captured(2).contains(QStringLiteral("rowspan"), Qt::CaseInsensitive)) {
                    // Заголовок: № / Ответы(colspan3) / HLP / Баллы.
                    const QStringList hw = {
                        QStringLiteral("40"), QStringLiteral("312"),
                        QStringLiteral("250"), QStringLiteral("69")};
                    QString newRow;
                    int cellLast = 0;
                    for (int i = 0; i < tds.size(); ++i) {
                        const QRegularExpressionMatch &td = tds.at(i);
                        newRow += rowInner.mid(cellLast, td.capturedStart() - cellLast);
                        QString attrs = setAttrWidth(td.captured(2), hw.at(i));
                        if (i == 1 && !attrs.contains(QStringLiteral("colspan"), Qt::CaseInsensitive)) {
                            attrs += QStringLiteral(" colspan='3'");
                        }
                    ...
                    // Подзаголовок «Вопрос № 1..3».
                    QString newRow;
                    int cellLast = 0;
                    for (int i = 0; i < tds.size(); ++i) {
                        const QRegularExpressionMatch &td = tds.at(i);
                        newRow += rowInner.mid(cellLast, td.capturedStart() - cellLast);
                        QString attrs = setAttrWidth(td.captured(2), QStringLiteral("104"));
                        newRow += td.captured(1) + attrs + td.captured(3) + td.captured(4)
                            + td.captured(5);
                        cellLast = td.capturedEnd();
                    }
                    ...
                    for (int i = 0; i < tds.size(); ++i) {
                        const QRegularExpressionMatch &td = tds.at(i);
                        newRow += rowInner.mid(cellLast, td.capturedStart() - cellLast);
                        QString attrs = setAttrWidth(td.captured(2), widths6.at(i));
                    ...
                    QString newRow;
                    int cellLast = 0;
                    const QStringList sw = {QStringLiteral("602"), QStringLiteral("69")};
                    for (int i = 0; i < tds.size(); ++i) {
                        const QRegularExpressionMatch &td = tds.at(i);
                        newRow += rowInner.mid(cellLast, td.capturedStart() - cellLast);
                        QString attrs = setAttrWidth(td.captured(2), sw.at(i));
                        if (i == 0 && !attrs.contains(QStringLiteral("colspan"), Qt::CaseInsensitive)) {
                            attrs += QStringLiteral(" colspan='5'");
                        }
                        QString cellHtml = td.captured(4);
                        if (i == 1
                            && !cellHtml.contains(
        ...
        while (it.hasNext()) {
            ...
            // Пропускаем 4+ колоночные таблицы (с № / картинкой и т.п.) — кроме известных сеток.
            {
                ...
                if (firstTr.hasMatch()) {
                    ...
                    // Не матчить по одному idb: у 3.1.21 тоже idb, но другая сетка («Ответы ребенка»).
        ...
        while (it.hasNext()) {
            ...
            while (trIt.hasNext()) {
                ...
                if (tds.size() == 3) {
                    ...
                    if (!processInner.isEmpty()) {
                        // Баннер в 3-кол. таблице — colspan=3 (не 2: иначе узкая пустая колонка).
                        rowInner = QStringLiteral(
                            "<td colspan='3' align='center' width='671' style='width:671px'>%1</td>")
                                       .arg(processInner);
                    } else if (tds.size() == 2) {
...
void ExerciseProtocol::forceProtocolDocumentTableWidths(QTextDocument *document, int widthPx) {
    ...
    for (QTextTable *table : tables) {
        ...
        if (cols >= 3 && table->rows() >= 1) {
            ...
            const bool isSummaryHeader =
                h0.contains(QStringLiteral("Методика"), Qt::CaseInsensitive)
                || h0.contains(QStringLiteral("Дата/специалист"), Qt::CaseInsensitive)
                || h0.contains(QStringLiteral("Дата / специалист"), Qt::CaseInsensitive);
        ...
        if (cols >= 2 && table->rows() >= 1) {
            for (int r = 0; r < table->rows(); ++r) {
                ...
                for (int c = 0; c < cols; ++c) {
                    ...
                    // Ячейка, поглощённая colspan предыдущей — cellAt всё равно валидна,
                    // но firstCursorPosition совпадает; читаем текст только у «своих».
                    if (probe.row() != r || probe.column() != c) {
                        continue;
                    }
                    if (readTableCellText(table, r, c)
                            .contains(QStringLiteral("Процесс выполнения"), Qt::CaseInsensitive)) {
                ...
                QTextTableCell cell = table->cellAt(r, processCol);
                if (!cell.isValid()) {
                    continue;
                }
                if (cell.columnSpan() < cols || processCol != 0) {
                    // Сначала очистить хвост строки, иначе merge оставит артефакты границ.
                    for (int c = cols - 1; c > processCol; --c) {
                        QTextTableCell other = table->cellAt(r, c);
                        if (!other.isValid() || other.row() != r || other.column() != c) {
                            continue;
                        }
                    ...
                    table->mergeCells(r, 0, 1, cols);
                    cell = table->cellAt(r, 0);
                }
                if (cell.isValid()) {
                    QTextTableCellFormat cellFmt = cell.format().toTableCellFormat();
                    cellFmt.setLeftPadding(0);
                    cellFmt.setRightPadding(0);
                ...
                break; // одна такая строка на таблицу
            }
        }
...
QString ExerciseProtocol::mergeProtocol1272EditorIntoStoredBody(
    ...
    for (QTextTable *table : tables) {
        ...
        for (int r = headerRow + 1; r < table->rows(); ++r) {
            ...
            // 3.1.21 и др.: только «Виды помощи» (без колонки «Характер деятельности»).
            if (activityCol < 0 && helpCol < 0) {
                continue;
            }
            const QString activity = activityCol >= 0
                ? readTableCellMultilineText(table, r, activityCol)
                : QString();
            const QString help = helpCol >= 0
                ? readTableCellMultilineText(table, r, helpCol)
                : QString();
